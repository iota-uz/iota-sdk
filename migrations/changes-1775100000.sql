-- +migrate Up
-- BiChat generation_runs: transient state for refresh-safe streaming (one active run per session).
...
-- +migrate Down
DROP INDEX IF EXISTS bichat.idx_generation_runs_tenant_session;
DROP INDEX IF EXISTS bichat.idx_generation_runs_session_active;
DROP TABLE IF EXISTS bichat.generation_runs;
