...
func NewAppletController(applet Applet, opts ...BuilderOption) *AppletController {
	config := applet.Config()
	builder := NewContextBuilder(config, opts...)

	return &AppletController{
		applet:  applet,
		builder: builder,
		logger:  builder.logger, // Extract logger from builder
	}
}
