...
func (h *AskUserQuestionHandler) Handle(ctx context.Context, event InterruptEvent) (bool, error) {
	...
	if err := json.Unmarshal(event.Data, &data); err != nil {
		return false, fmt.Errorf("AskUserQuestionHandler.Handle: %w", err)
	}

	// User interaction is handled by the production module (modules/bichat)
	// via ChatService.ResumeWithAnswer() which loads the checkpoint and
	// continues execution with the user's answers.
	// This function just validates the interrupt event structure.
