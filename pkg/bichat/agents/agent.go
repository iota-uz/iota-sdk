...
// IsolationLevel defines how much context a child agent can access from its parent.
// This controls information flow in multi-agent delegation scenarios.
type IsolationLevel int

const (
	// Isolated means the child agent has no access to parent context.
	// The child starts with a clean slate and only receives the delegated task input.
	// Use this for security-sensitive operations or when child should not see parent history.
	Isolated IsolationLevel = iota

	// ReadParent means the child can read parent context but cannot modify it.
	// The child sees parent's conversation history but its own messages/tools
	// don't affect the parent's context.
	// Use this when child needs context but should remain sandboxed.
	ReadParent

	// FullAccess means the child has full read/write access to parent context.
	// Child's messages and tool calls become part of parent's history.
	// Use this for cooperative multi-agent workflows where agents share state.
	FullAccess
)
...
type AgentMetadata struct {
	...
	// Isolation defines the context access level for this agent when delegated.
	// This controls how much information flows between parent and child.
...
func NewBaseAgent(opts ...AgentOption) *BaseAgent {
	agent := &BaseAgent{
		metadata: AgentMetadata{
			Isolation:        Isolated, // Default to most restrictive
...
// WithIsolation sets the context isolation level for this agent.
// This controls how much information flows between parent and child.
//
// Example:
//
//	WithIsolation(Isolated)        // No access to parent
//	WithIsolation(ReadParent)      // Read-only access
//	WithIsolation(FullAccess)      // Full read/write
func WithIsolation(level IsolationLevel) AgentOption {
	return func(a *BaseAgent) {
		a.metadata.Isolation = level
	}
}
