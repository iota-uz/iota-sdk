	...
	if params.Question == "" {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				...
				Hints:   []string{tools.HintCheckRequiredFields, "Provide search terms for validated query lookup"},
			},
		}, nil
	}
	...
	if params.Question == "" {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				Code:    string(tools.ErrCodeInvalidRequest),
				Message: "question parameter is required",
				Hints:   []string{tools.HintCheckRequiredFields, "Describe the business question this query answers"},
			},
		}, nil
	}
	if params.SQL == "" {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				...
				Hints:   []string{tools.HintCheckRequiredFields, "Provide the validated SQL query"},
			},
		}, nil
	}
	if params.Summary == "" {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				...
				Hints:   []string{tools.HintCheckRequiredFields, "Provide a brief description of what the query does"},
			},
		}, nil
	}
	if len(params.TablesUsed) == 0 {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				...
				Hints:   []string{tools.HintCheckRequiredFields, "Provide list of tables referenced in the query"},
			},
		}, nil
	}
	...
	if !strings.HasPrefix(normalizedSQL, "SELECT") && !strings.HasPrefix(normalizedSQL, "WITH") {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				...
				Hints:   []string{tools.HintCheckFieldTypes, "Ensure the SQL query starts with SELECT or WITH"},
			},
		}, nil
	}
