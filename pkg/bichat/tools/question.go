...
func (t *AskUserQuestionTool) Call(ctx context.Context, input string) (string, error) {
	...
	if len(params.Questions) == 0 {
		return "", serrors.E(op, "at least one question is required")
	}
	if len(params.Questions) > 4 {
		return "", serrors.E(op, "maximum 4 questions allowed")
	}

	// Validate each question
	for i, q := range params.Questions {
		if q.Question == "" {
			return "", serrors.E(op, fmt.Sprintf("question[%d]: question text is required", i))
		}
		if q.Header == "" {
			return "", serrors.E(op, fmt.Sprintf("question[%d]: header is required", i))
		}
		if len(q.Header) > 12 {
			return "", serrors.E(op, fmt.Sprintf("question[%d]: header exceeds 12 characters", i))
		}
		if len(q.Options) < 2 {
			return "", serrors.E(op, fmt.Sprintf("question[%d]: at least 2 options required", i))
		}
		if len(q.Options) > 4 {
			return "", serrors.E(op, fmt.Sprintf("question[%d]: maximum 4 options allowed", i))
		}

		// Validate each option
		for j, opt := range q.Options {
			if opt.Label == "" {
				return "", serrors.E(op, fmt.Sprintf("question[%d].option[%d]: label is required", i, j))
			}
			if opt.Description == "" {
				return "", serrors.E(op, fmt.Sprintf("question[%d].option[%d]: description is required", i, j))
			}
		}
	}
