	...
	if err != nil {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				Code:    string(tools.ErrCodeInvalidRequest),
				Message: err.Error(),
				Hints:   []string{tools.HintCheckRequiredFields, "Use action=\"list\" to discover available artifact names and IDs"},
			},
		}, nil
	}
	...
	if err != nil {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				...
				Hints:   []string{tools.HintCheckConnection, tools.HintRetryLater},
			},
		}, err
	}
