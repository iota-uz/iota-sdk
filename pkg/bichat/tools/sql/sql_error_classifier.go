...
func ClassifySQLError(err error) *SQLErrorDiagnosis {
	if err == nil {
		return &SQLErrorDiagnosis{
			...
			Hints:      []string{tools.HintCheckSQLSyntax},
		}
	}
	...
	if !errors.As(err, &pgErr) {
		...
		return &SQLErrorDiagnosis{
			...
			Hints:      []string{tools.HintCheckSQLSyntax, tools.HintVerifyTableNames},
		}
	}
...
func classifyUndefinedColumn(pgErr *pgconn.PgError) *SQLErrorDiagnosis {
	...
	hints := []string{tools.HintCheckColumnExists, tools.HintUseSchemaDescribe}
	...
	return &SQLErrorDiagnosis{
		Code:       tools.ErrCodeColumnNotFound,
		Message:    pgErr.Message,
		Table:      table,
		Column:     column,
...
func classifyUndefinedTable(pgErr *pgconn.PgError) *SQLErrorDiagnosis {
	...
	hints := []string{tools.HintUseSchemaList, tools.HintVerifyTableNames}
...
func classifyAmbiguousColumn(pgErr *pgconn.PgError) *SQLErrorDiagnosis {
	...
	var column, suggestion string
	hints := []string{tools.HintDisambiguateColumn, tools.HintUseSchemaDescribe}
	...
	return &SQLErrorDiagnosis{
		Code:       tools.ErrCodeAmbiguousColumn,
		Message:    pgErr.Message,
		Column:     column,
		Suggestion: suggestion,
		Hints:      hints,
