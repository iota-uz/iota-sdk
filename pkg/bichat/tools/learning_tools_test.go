...
func TestSearchLearningsTool_NoResults(t *testing.T) {
	...
	if !strings.Contains(result, string(ErrCodeNoData)) {
		t.Errorf("expected ErrCodeNoData in result, got: %s", result)
	}
...
func TestSearchLearningsTool_MissingQuery(t *testing.T) {
	...
	if !strings.Contains(result, string(ErrCodeInvalidRequest)) {
		t.Errorf("expected ErrCodeInvalidRequest in result, got: %s", result)
	}
