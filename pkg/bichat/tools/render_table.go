...
func (t *RenderTableTool) CallStructured(ctx context.Context, input string) (*types.ToolResult, error) {
	...
	normalizedQuery := normalizeSQL(params.SQL)
	if normalizedQuery == "" {
		return &types.ToolResult{
			CodecID: types.CodecToolError,
			Payload: types.ToolErrorPayload{
				...
				Hints:   []string{HintCheckRequiredFields, HintCheckSQLSyntax},
			},
		}, nil
	}
	...
	if pageSize < 1 || pageSize > renderTableMaxPageSize {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				...
				Hints:   []string{HintCheckFieldTypes},
			},
		}, nil
	}
	...
	if maxRows < 1 || maxRows > renderTableHardMaxRows {
		return &types.ToolResult{
			...
			Payload: types.ToolErrorPayload{
				...
				Hints:   []string{HintCheckFieldTypes, HintAddLimitClause},
			},
		}, nil
	}
		...
		if exportErr != nil {
			return &types.ToolResult{
				...
				Payload: types.ToolErrorPayload{
					...
					Hints:   []string{HintRetryLater, HintServiceMayBeDown},
				},
