...
func TestFormatSQLDiagnosis(t *testing.T) {
	t.Parallel()

	diagnosis := &SQLErrorDiagnosis{
		Code:       ErrCodeColumnNotFound,
		Message:    `column "user_name" does not exist`,
		Column:     "user_name",
		Suggestion: "Use schema_describe to verify available columns.",
		Hints:      []string{HintCheckColumnExists, HintUseSchemaDescribe},
	}

	result := FormatSQLDiagnosis(diagnosis)
	...
	if !strings.Contains(result, "code") {
		t.Errorf("expected 'code' field in result, got: %s", result)
	}
	if !strings.Contains(result, "message") {
		t.Errorf("expected 'message' field in result, got: %s", result)
	}
	if !strings.Contains(result, "suggestion") {
		t.Errorf("expected 'suggestion' field in result, got: %s", result)
	}
	if !strings.Contains(result, "hints") {
		t.Errorf("expected 'hints' field in result, got: %s", result)
	}
	if !strings.Contains(result, string(ErrCodeColumnNotFound)) {
		t.Errorf("expected error code in result, got: %s", result)
	}
}

func TestFormatSQLDiagnosis_Nil(t *testing.T) {
	t.Parallel()

	result := FormatSQLDiagnosis(nil)

	if result == "" {
		t.Error("expected non-empty result")
	}
	if !strings.Contains(result, string(ErrCodeQueryError)) {
		t.Errorf("expected ErrCodeQueryError in result, got: %s", result)
	}
	if !strings.Contains(result, "error") {
		t.Errorf("expected 'error' key in result, got: %s", result)
	}
