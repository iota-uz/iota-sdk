...
- CSV пишется одной реализацией `pkg/excel` — `CSVExporter` для `DataSource` и
  ...
  апостроф перед `=`, `@`, табуляцией, CR, а также перед `-`, за которым идёт не
  одно число, и перед `+`, за которым идёт не число или телефон. Форматированные
  суммы (`-1 234,56`), прочерк `-` и телефоны (`+998 (90) 123-45-67`) не
  меняются; `-1-1` таблица посчитала бы как выражение, поэтому он экранируется.
