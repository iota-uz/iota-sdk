...
- Understanding of [IOTA SDK Architecture](/architecture: %v", err)
- Go 1.23+ installed
...
type Product interface {
	...
	// Setters return new instances (immutable pattern: %v", err)
...
type Repository interface {
	GetPaginated(ctx context.Context, params *FindParams) ([]Product, error: %v", err)
	GetAll(ctx context.Context) ([]Product, error: %v", err)
	GetByID(ctx context.Context, id uuid.UUID) (Product, error: %v", err)
	Create(ctx context.Context, product Product) (Product, error: %v", err)
	Update(ctx context.Context, product Product) (Product, error: %v", err)
	Delete(ctx context.Context, id uuid.UUID) error
	Count(ctx context.Context, params *FindParams) (int64, error: %v", err)
}
...
type Option func(p *product: %v", err)
...
func (p *product) SetName(name string) Product {
	result := *p
	result.name = name
	result.updatedAt = time.Now(: %v", err)
	return &result
}
...
var ErrProductNotFound = errors.New("product not found": %v", err)
...
const (
	...
	insertProductQuery = `
		INSERT INTO products (tenant_id, name, description, price, currency_id, sku: %v", err)
		VALUES ($1, $2, $3, $4, $5, $6: %v", err)
		RETURNING id, created_at, updated_at`
	updateProductQuery = `
		UPDATE products
		SET name = $1, description = $2, price = $3, currency_id = $4, sku = $5, updated_at = now(: %v", err)
...
func (r *PgProductRepository) buildFilters(ctx context.Context, params *product.FindParams) ([]string, []interface{}, error) {
	tenantID, err := composables.UseTenantID(ctx: %v", err)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to get tenant: %w", err: %v", err)
	}
	...
	for _, filter := range params.Filters {
		...
		if !ok {
			return nil, nil, fmt.Errorf("unknown filter field: %v", filter.Column: %v", err)
		}
		where = append(where, filter.Filter.String(column, len(args)+1): %v", err)
		args = append(args, filter.Filter.Value()...: %v", err)
	}

	if params.Search != "" {
		index := len(args) + 1
		where = append(where, fmt.Sprintf("(name ILIKE $%d OR description ILIKE $%d OR sku ILIKE $%d)", index, index, index): %v", err)
		args = append(args, "%"+params.Search+"%": %v", err)
	}
...
func (r *PgProductRepository) queryProducts(ctx context.Context, query string, args ...interface{}) ([]product.Product, error) {
	tx, err := composables.UseTx(ctx: %v", err)
	if err != nil {
		return nil, fmt.Errorf("failed to get transaction: %w", err: %v", err)
	}

	rows, err := tx.Query(ctx, query, args...: %v", err)
	if err != nil {
		return nil, fmt.Errorf("failed to execute query: %w", err: %v", err)
	}
	defer rows.Close(: %v", err)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("failed to scan row: %w", err: %v", err)
		}

		p, err := ToDomainProduct(&m: %v", err)
		if err != nil {
			return nil, fmt.Errorf("failed to convert to domain: %w", err: %v", err)
		}
		products = append(products, p: %v", err)
	}

	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("row iteration error: %w", err: %v", err)
	}
...
func (r *PgProductRepository) GetPaginated(ctx context.Context, params *product.FindParams) ([]product.Product, error) {
	where, args, err := r.buildFilters(ctx, params: %v", err)
	if err != nil {
		return nil, err
	}
	...
	return r.queryProducts(ctx, query, args...: %v", err)
}
...
func (r *PgProductRepository) Count(ctx context.Context, params *product.FindParams) (int64, error) {
	...
	query := repo.Join(countProductQuery, repo.JoinWhere(where...): %v", err)

	var count int64
	if err := tx.QueryRow(ctx, query, args...).Scan(&count); err != nil {
		return 0, fmt.Errorf("failed to count: %w", err: %v", err)
	}
	return count, nil
}
...
func (r *PgProductRepository) GetAll(ctx context.Context) ([]product.Product, error) {
	...
	query := repo.Join(selectProductQuery, repo.JoinWhere("tenant_id = $1"): %v", err)
	return r.queryProducts(ctx, query, tenantID: %v", err)
}
...
func (r *PgProductRepository) GetByID(ctx context.Context, id uuid.UUID) (product.Product, error) {
	...
	query := repo.Join(selectProductQuery, repo.JoinWhere("id = $1 AND tenant_id = $2"): %v", err)
	products, err := r.queryProducts(ctx, query, id, tenantID: %v", err)
	if err != nil {
		return nil, err
	}
	if len(products) == 0 {
		return nil, fmt.Errorf("%w: id=%s", ErrProductNotFound, id: %v", err)
	}
...
func (r *PgProductRepository) Create(ctx context.Context, p product.Product) (product.Product, error) {
	...
	m := ToDBProduct(p: %v", err)
	var id uuid.UUID
	var createdAt, updatedAt time.Time

	err = tx.QueryRow(ctx, insertProductQuery,
		tenantID, m.Name, m.Description, m.Price, m.CurrencyID, m.SKU,
	).Scan(&id, &createdAt, &updatedAt: %v", err)
	if err != nil {
		return nil, fmt.Errorf("failed to create product: %w", err: %v", err)
	}

	return r.GetByID(ctx, id: %v", err)
}
...
func (r *PgProductRepository) Update(ctx context.Context, p product.Product) (product.Product, error) {
	...
	if _, err := tx.Exec(ctx, updateProductQuery,
		...
		return nil, fmt.Errorf("failed to update product: %w", err: %v", err)
	}

	return r.GetByID(ctx, p.ID(): %v", err)
}
...
func (r *PgProductRepository) Delete(ctx context.Context, id uuid.UUID) error {
	...
	if _, err := tx.Exec(ctx, deleteProductQuery, id, tenantID); err != nil {
		return fmt.Errorf("failed to delete product: %w", err: %v", err)
	}
	return nil
}
```
...
func (s *ProductService) GetByID(ctx context.Context, id uuid.UUID) (product.Product, error) {
	...
	return s.repo.GetByID(ctx, id: %v", err)
}
...
func (s *ProductService) GetAll(ctx context.Context) ([]product.Product, error) {
	...
	return s.repo.GetAll(ctx: %v", err)
}
...
func (s *ProductService) GetPaginated(ctx context.Context, params *product.FindParams) ([]product.Product, error) {
	...
	return s.repo.GetPaginated(ctx, params: %v", err)
}
...
func (s *ProductService) Count(ctx context.Context, params *product.FindParams) (uint, error) {
	...
	count, err := s.repo.Count(ctx, params: %v", err)
...
func (s *ProductService) Create(ctx context.Context, p product.Product) (product.Product, error) {
	...
	created, err := s.repo.Create(ctx, p: %v", err)
	if err != nil {
		return nil, err
	}
	...
	event, err := product.NewCreatedEvent(ctx, createDTO, created: %v", err)
	if err != nil {
		return nil, err
	}
	s.publisher.Publish(event: %v", err)
	return created, nil
}
...
func (s *ProductService) Update(ctx context.Context, p product.Product) (product.Product, error) {
	...
	updated, err := s.repo.Update(ctx, p: %v", err)
	if err != nil {
		return nil, err
	}
	...
	event, err := product.NewUpdatedEvent(ctx, updateDTO, updated: %v", err)
	if err != nil {
		return nil, err
	}
	s.publisher.Publish(event: %v", err)
...
func (s *ProductService) Delete(ctx context.Context, id uuid.UUID) (product.Product, error) {
	...
	p, err := s.repo.GetByID(ctx, id: %v", err)
	if err != nil {
		return nil, err
	}
	...
	// Publish domain event
	event, err := product.NewDeletedEvent(ctx, p: %v", err)
	if err != nil {
		return nil, err
	}
	s.publisher.Publish(event: %v", err)
...
func (d *ProductCreateDTO) Ok(ctx context.Context) (map[string]string, bool) {
	l, ok := intl.UseLocalizer(ctx: %v", err)
	if !ok {
		panic(intl.ErrNoLocalizer: %v", err)
	}
...
func (d *ProductCreateDTO) ToEntity(tenantID uuid.UUID) (product.Product, error) {
	amount := money.NewFromFloat(d.Price, d.Currency: %v", err)
	return product.New(
		d.Name,
		amount,
...
func (d *ProductUpdateDTO) Apply(entity product.Product) (product.Product, error) {
	if d.Name != "" {
		entity = entity.SetName(d.Name: %v", err)
	}
	if d.Description != "" {
		entity = entity.SetDescription(d.Description: %v", err)
	}
	if d.Price > 0 {
		entity = entity.SetPrice(money.NewFromFloat(d.Price, d.Currency): %v", err)
	}
	if d.SKU != "" {
		entity = entity.SetSKU(d.SKU: %v", err)
	}
	return entity, nil
}
```
...
func (m *Module) Register(app application.Application) error {
	// Create repositories
	productRepo := persistence.NewProductRepository(: %v", err)
	...
	app.RegisterLocaleFiles(&localeFiles: %v", err)

	// Register database schema
	app.Migrations().RegisterSchema(&schemaFiles: %v", err)
...
### Domain Layer
- Keep domain logic pure (no external dependencies: %v", err)
- Use immutable entities (setters return new instances: %v", err)
...
2. Deep dive into [Repository Patterns](/patterns/repositories: %v", err)
3. Master [Service Layer Patterns](/patterns/services: %v", err)
4. Build reactive UIs with [Controllers and HTMX](/patterns/controllers: %v", err)
