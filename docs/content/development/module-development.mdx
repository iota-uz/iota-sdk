...
Create `modules/productcatalog/component.go`:
...
func (c *component) Descriptor() composition.Descriptor {
	return composition.Descriptor{Name: "productcatalog"}
}

func (c *component) Build(builder *composition.Builder) error {
	// Static contributions: zero closures for constant data.
	composition.AddLocales(builder, &localeFiles)
	composition.AddNavItems(builder, NavItems...)
	composition.AddQuickLinks(builder, spotlight.NewQuickLink(NavItems[0].Name, NavItems[0].Href))

	// ProvideFuncAs registers both an interface key (product.Repository)
	// and the constructor's concrete return type as provider keys, so
	// consumers depending on either resolve the exact same instance. Use
	// this when persistence.NewProductRepository returns a concrete type
	// like *persistence.ProductRepositoryImpl — if the constructor already
	// returns the interface, plain ProvideFunc is the right helper
	// (ProvideFuncAs rejects an identical concrete+interface at
	// registration time to prevent a redundant bridge).
	composition.ProvideFuncAs[product.Repository](builder, persistence.NewProductRepository)

	// ProvideFunc inspects NewProductService's signature, resolves each
	// parameter from the container by type (product.Repository and
	// eventbus.EventBus — the latter comes from the engine's auto-providers),
	// and registers *services.ProductService as a provider.
	composition.ProvideFunc(builder, services.NewProductService)

	if builder.Context().HasCapability(composition.CapabilityAPI) {
		// ContributeControllersFunc uses the same reflection injector for
		// controller factories. Types in the parameter list are resolved
		// from the container at materialization time.
		composition.ContributeControllersFunc(builder, controllers.NewProductController)
	}
