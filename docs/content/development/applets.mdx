...
When you use **`just dev <name>`**, the Go dev runner writes **`applet-dev.json`** in the applet's Vite directory. The frontend helpers (`createAppletViteConfig`, `getAppletAssetsBase`, `getAppletVitePort`) read this file and use it for `base`, port, and proxy—so the manifest is the single source of truth; env vars below are the fallback when not using the dev runner.

The Go dev runner also sets these env vars for compatibility:

- `APPLET_ASSETS_BASE` — must end with `/`; Vite uses it as `base` when no manifest is present.
- `APPLET_VITE_PORT` — Vite dev server port (used when no manifest is present).
...
`X-Forwarded-Host` is trusted by default for RPC same-origin checks, which is safe for standard reverse-proxy deployments. To disable it:

- `Config.RPC.TrustForwardedHost = boolPtr(false)`
