...
- Enable with `IOTA_APPLET_ENGINE_BICHAT=bun`
...
- `IOTA_APPLET_ENGINE_BICHAT_KV_BACKEND=redis`
- `IOTA_APPLET_ENGINE_REDIS_URL=redis://localhost:6379`

If Redis backend is enabled without `IOTA_APPLET_ENGINE_REDIS_URL`, app startup fails fast.

### Slice 2 kickoff: Postgres DB backend (BiChat)

An optional PostgreSQL backend is available for BiChat server-only `db.*` methods.

- `IOTA_APPLET_ENGINE_BICHAT_DB_BACKEND=postgres`
...
- `IOTA_APPLET_ENGINE_BICHAT_JOBS_BACKEND=postgres`
- `IOTA_APPLET_ENGINE_BICHAT_JOBS_RUNNER=1` to enable the background poller that dispatches queued jobs to Bun (`/__job`).

This backend stores jobs in `applet_engine_jobs` (see migrations). If enabled without an initialized application DB pool, app startup fails fast.
