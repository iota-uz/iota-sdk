...
2. **Composition Container** - Typed service access via `composition.ProvideFunc` (constructors with typed parameters) and `composition.Resolve[T](container)` from inside Contribute\* closures
...
| Data access | sqlx + pgx | Queries and driver |
