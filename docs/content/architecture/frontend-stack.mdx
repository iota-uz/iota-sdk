...
description: 'Templ/HTMX server pages and Solid client routes'
---

# Frontend Stack

IOTA SDK uses server-rendered HTML with progressive enhancement for
server-owned pages and SolidJS for routes with substantial client-owned
interaction state. See the [Solid client runtime](./solid-client-runtime) for
the ownership and persistence boundaries.
...
| **SolidJS** | Client routes | Direct reactive state for complex interaction |
...
sequenceDiagram
    ...
    Browser->>HTMX: User action (hx-post, etc.)
    HTMX->>Controller: AJAX request + HX headers
    Controller->>Controller: Check htmx.IsHxRequest(r)
