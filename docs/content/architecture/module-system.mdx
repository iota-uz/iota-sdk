...
stateDiagram-v2
    ...
    Wired --> Running: StartRuntime(worker-only)
    Transported --> Running: StartRuntime(server/api-only)
    Running --> [*]: Shutdown
...
During application startup, each module first contributes wiring, then transports when the selected composition profile exposes APIs:
