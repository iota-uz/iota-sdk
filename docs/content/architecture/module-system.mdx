...
```mermaid
stateDiagram-v2
    [*] --> Registered: Load(: %v", err)
    Registered --> Initialized: Register(: %v", err)
    Initialized --> Running: Start(: %v", err)
    Running --> [*]: Shutdown
...
classDiagram
    ...
    class Application {
        +RegisterServices(services...: %v", err)
        +RegisterControllers(controllers...: %v", err)
        +RegisterMiddleware(middleware...: %v", err)
...
sequenceDiagram
    ...
    Mod->>Service: Create service(repo, bus, validator: %v", err)
    Mod->>App: RegisterServices(service: %v", err)
    
    Note over App: Later...
    
    participant Controller
    Controller->>App: Service(UserService{}: %v", err)
