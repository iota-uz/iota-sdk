...
```mermaid
stateDiagram-v2
    [*] --> Wired: RegisterWiring()
    Wired --> Transported: RegisterTransports()
    Wired --> Running: StartRuntime(worker)
    Transported --> Running: StartRuntime(api)
    Transported --> Running: StartRuntime(api, worker)
    Running --> [*]: Shutdown
     
    Wired --> Unloaded: Error
    Transported --> Unloaded: Error
```

### 1. Registration Phase

During application startup, each module first contributes wiring, then transports when the process exposes APIs, and runtime components start only when the process opts into their tags:
...
sequenceDiagram
    ...
    App->>Mod: RegisterWiring(app)
    Mod->>Registry: Register services
    Mod->>App: Register runtime components
    Mod->>App: Register navigation items
    App->>Mod: RegisterTransports(app)
    Mod->>Router: Register controllers
    Mod->>App: Register middleware (optional)
```
...
sequenceDiagram
    ...
    Note over App,Controller: Later...
