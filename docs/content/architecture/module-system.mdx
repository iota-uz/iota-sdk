...
| `composition.ContributeEventHandler(b, handler)` | Auto-subscribed eventbus handler | Infrastructure |
...
4. Most core services (`*pgxpool.Pool`, `eventbus.EventBus`, `*i18n.Bundle`, `*logrus.Logger`, `application.Application`, `spotlight.Service`) are auto-registered by `Engine.Compile`. Drop `composition.RequireApplication` calls that only pulled one of these, and let the injector resolve them via `ProvideFunc` / `ContributeControllersFunc`.
