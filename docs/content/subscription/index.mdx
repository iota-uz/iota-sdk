...
For Stripe-driven entitlement changes that are written to PostgreSQL, use the
bridge evaluator so middleware reads current repository state instead of only
in-memory grants:
...
router.Use(subscription.EnforceLimit(repoEvaluator, "drivers"))
```
...
result, err := svc.EvaluateLimit(
  ctx,
  subject,
  mustQuotaKey("drivers", "", subscription.WindowNone),
)
if err != nil {
  return err
}
if !result.Allowed {
  return subscription.LimitExceededError{
    Quota: mustQuotaKey("drivers", "", subscription.WindowNone),
    Current: result.Current,
    Limit: result.Limit,
  }
}

func mustQuotaKey(resource, dimension string, window subscription.Window) subscription.QuotaKey {
  key, err := subscription.NewQuotaKey(resource, dimension, window)
  if err != nil {
    panic(err)
  }
  return key
