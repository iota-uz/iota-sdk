...
**⚠️ Note on Verification Claims:**

The `email_verified` and `phone_number_verified` claims are currently set to `false` for all users. Email and phone verification systems are not yet implemented (planned for Phase 2).

**Do not use these claims for authorization decisions.** Always implement your own verification flow if you need to ensure email/phone ownership.
