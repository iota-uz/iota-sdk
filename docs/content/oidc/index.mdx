...
OpenID Connect (OIDC) authentication and authorization module for IOTA SDK. The component is included in `modules.Components()` and **activates automatically** when required settings are configured (`OIDC_ISSUER_URL` and `OIDC_CRYPTO_KEY`); no separate registration step is needed.
