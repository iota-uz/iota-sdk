...
Other modules access Core services by declaring them as constructor
parameters. The reflection injector resolves each parameter from the
composition container by type, so there is no explicit lookup ceremony:

```mermaid
graph LR
    A[Finance Component] -->|"ProvideFunc(NewExpenseService)"| B[UserService]
    C[Warehouse Component] -->|"ProvideFunc(NewProductService)"| D[UploadService]
    E[CRM Component] -->|"ProvideFunc(NewClientService)"| F[SessionService]
```

```go
// modules/finance/component.go
composition.ProvideFunc(builder, services.NewExpenseService) // takes *UserService, upload.Repository, ...
```
