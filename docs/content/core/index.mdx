...
The Core module is included in `BuiltInModules` and is usually loaded through `application.ApplyProfile(ctx, app, application.CompositionProfileServer, modules.BuiltInModules...)`. If you need more control, call `application.Wire(app, modules.BuiltInModules...)`, then `application.RegisterTransports(...)`, and start runtime separately with the profile your process needs. There is no `app.RegisterModule(module)`; composition happens through the application helpers.
