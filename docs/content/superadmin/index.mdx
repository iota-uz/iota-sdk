...
- Tenant lifecycle management (create via documented REST endpoints; suspend/delete may be handled by internal or deployment-specific workflows)
...
The SuperAdmin app (separate binary, port 4000) registers (all protected by `RequireSuperAdmin()`):

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/` | GET | Dashboard (overview page) |
| `/metrics` | GET | Dashboard metrics (HTMX partial) |
| `/superadmin/tenants` | GET | List tenants |
| `/superadmin/tenants` | POST | Create tenant |
| `/superadmin/tenants/export` | POST | Export tenants |
| `/superadmin/tenants/{id}/users` | GET | List users for a tenant |
| `/superadmin/tenants/{id}/users/{userId}/reset-password` | POST | Reset user password |

There is no tenant GET-by-id, PUT update, or suspend route in the current implementation. If suspend/delete behavior exists in your deployment, it is handled outside these REST routes. Access is enforced by `RequireSuperAdmin()` middleware (not RBAC constants like `superadmin.tenants.view`), and tenant users listing remains available via `/superadmin/tenants/{id}/users`.
