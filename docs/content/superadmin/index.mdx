...
There are no GET-by-id, PUT update, or suspend routes in the current implementation. If suspend or delete are enabled in your deployment, they are handled outside the REST routes documented above. Access is enforced by the superadmin role (e.g. `RequireSuperAdmin()` middleware), not by RBAC permission constants like `superadmin.tenants.view`.
