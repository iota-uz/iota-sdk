...
There are no GET-by-id, PUT update, or suspend routes in the current implementation. Access is enforced by the superadmin role (e.g. `RequireSuperAdmin()` middleware), not by RBAC permission constants like `superadmin.tenants.view`.
