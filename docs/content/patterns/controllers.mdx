...
Controllers are plain structs whose constructor receives typed services.
There is no `application.Application` field anymore — global services such
as the i18n bundle, pool, event bus, and user session are installed as
request-scoped middleware by the server bootstrap (`pkg/server/builder.go`)
and resolved per request via `composables.*`.
...
func NewExpensesController(expenseService *services.ExpenseService) application.Controller {
...
After the composition refactor, controllers are constructed via their
public constructor and no longer hold an `application.Application` field.
Mock the typed services you care about and inject them via the
constructor — per-handler dependencies (`di.H`-style) arrive through
the handler's function parameters at request time.

```go
func TestExpenseController_List(t *testing.T) {
	mockService := new(MockExpenseService)

	// Call the real constructor — same shape the component uses.
	ctrl := controllers.NewExpensesController(mockService, "/finance/expenses")
	...
	t.Run("returns list", func(t *testing.T) {
		...
		// Register the controller on a throwaway router so we can
