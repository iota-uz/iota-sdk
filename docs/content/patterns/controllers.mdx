...
Controllers are plain structs with a zero-argument constructor. Services
are **not** stored on the struct — they arrive as typed parameters on each
handler method via the `di.H` wrapper. Global services (i18n bundle, pool,
event bus, user session) are installed as request-scoped middleware by
`pkg/server/builder.go` and resolved per request via `composables.*`.
...
`Descriptor().ID` is the stable controller identity used by composition. It is
not derived from the mux path. `Descriptor().Order` controls registration order,
and `Descriptor().Routes` gives composition enough metadata to validate route
collisions before the router is mounted. Override another controller by setting
`Descriptor().Replaces`; do not remove controllers by path strings.

Applet engine controllers follow the same SDK controller descriptor model when
they are mounted into the application. New SDK-facing controller examples should
implement `Descriptor()`, not `Key()`.

### Route Registration

Register routes with middleware. The global i18n, logger, tenant, and
session middlewares are already installed by the server builder; each
controller only attaches the access-control middlewares it needs.
...
Some modules register a controller by having another controller call its `Register` on a sub-router (e.g. the Finance module mounts the expense controller under the financial overview router rather than registering it directly in `component.go`).
...
func TestExpenseController_Create(t *testing.T) {
	...
	env := itf.Setup(t, itf.WithComponents(finance.NewComponent()))
	defer env.Teardown()
...
### Unit Test with ITF Suite

After the composition refactor, controllers have zero-argument constructors
and handler dependencies arrive via `di.H` parameter injection at request
time. Use the ITF suite to wire the full composition container and exercise
