...
Services are wired together during the module wiring phase. The reflection
injector (`composition.ProvideFunc`) resolves each constructor parameter from
the container by type, so the component body reads as a straight list of
constructors:
...
func (c *component) Build(builder *composition.Builder) error {
	// Repositories — persistence constructors already return the
	// domain-level interface, so no concrete/interface bridge is needed.
	composition.ProvideFunc(builder, corepersistence.NewUploadRepository)
	...
	// Services — parameters are resolved by type from the providers above
	// plus the auto-provided eventbus.EventBus.
...
If a constructor returns a concrete type but you want the provider keyed by
an interface (for example, when another module registers a stub implementation
under the same interface key), use `composition.ProvideFuncAs[Interface]`:

```go
composition.ProvideFuncAs[storage.Backend](builder, persistence.NewS3Backend)
```

`ProvideFuncAs` panics at compile time if the constructor already returns the
interface — in that case use the plain `ProvideFunc`.
...
func (c *component) Build(builder *composition.Builder) error {
	composition.ContributeEventHandlerFunc(builder,
		func(analytics *AnalyticsService) any {
			return func(event *expense.CreatedEvent) error {
				// Handle expense creation — e.g., update analytics.
				return analytics.RecordExpenseCreated(event)
			}
		},
	)
	return nil
...
`ContributeEventHandlerFunc` resolves the factory's argument from the container
and invokes it during `Engine.Compile.materialize` — the same phase that
resolves every other contribution — so any resolution error, nil-handler
panic, or factory side effect surfaces at compile time, not lazily at Start.
Only the `eventbus.Subscribe` call is deferred to hook start; the matching
`Unsubscribe` runs from the returned `StopFn` during shutdown. Callers do
not need to write `Subscribe`/`Unsubscribe` bookkeeping.

For cases where a long-lived background worker needs a custom teardown, use
`ContributeHooks` with the current signature:

```go
composition.ContributeHooks(builder, func(container *composition.Container) ([]composition.Hook, error) {
	worker, err := composition.Resolve[*BackgroundWorker](container)
	if err != nil {
		return nil, err
	}
	return []composition.Hook{{
		Name: "expense-background-worker",
