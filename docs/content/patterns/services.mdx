...
invokes it, and wires the returned handler to the event bus on hook start. The
subscription is torn down automatically when the runtime shuts down — callers
do not need to write `Subscribe`/`Unsubscribe` bookkeeping.
