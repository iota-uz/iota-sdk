...
Services are wired together during the module wiring phase:
...
func (c *component) Build(builder *composition.Builder) error {
	...
	categoryRepo := composition.Use[expensecategory.Repository]()
	...
	composition.Provide[upload.Repository](builder, func() upload.Repository {
		return corepersistence.NewUploadRepository()
	})
	composition.Provide[expensecategory.Repository](builder, func() expensecategory.Repository {
	...
	composition.Provide[*services.MoneyAccountService](builder, func(container *composition.Container) (*services.MoneyAccountService, error) {
		...
		return services.NewMoneyAccountService(
			resolvedMoneyAccountRepo,
			resolvedTransactionRepo,
	...
	composition.Provide[*services.ExpenseService](builder, func(container *composition.Container) (*services.ExpenseService, error) {
		resolvedCategoryRepo, err := categoryRepo.Resolve(container)
		if err != nil {
			return nil, err
		}
		resolvedTransactionRepo, err := transactionRepo.Resolve(container)
		...
		return services.NewExpenseService(
			persistence.NewExpenseRepository(resolvedCategoryRepo, resolvedTransactionRepo),
			builder.Context().EventPublisher(),
			resolvedMoneyAccountService,
			resolvedUploadRepo,
		), nil
	})
...
func (c *component) Build(builder *composition.Builder) error {
	composition.ContributeHooks(builder, func(*composition.Container) ([]composition.Hook, error) {
		return []composition.Hook{{
			Name: "expense-created-subscriber",
			Start: func(context.Context, *composition.Container) error {
				builder.Context().EventPublisher().Subscribe(
