...
The module is configured via `ModuleConfig` in Go. **Required:** you must configure attachment storage (or explicitly disable it for tests). Module registration can **fail** at startup if configuration is invalid; check the error from `module.RegisterWiring(app)`, `module.RegisterTransports(app)`, and `app.StartRuntime(...)` as you compose the process.
...
if err := module.RegisterWiring(app); err != nil {
    log.Fatalf("Failed to wire BiChat: %v", err)
}
