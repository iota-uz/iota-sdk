...
The module is configured via `ModuleConfig` in Go. **Required:** you must configure attachment storage (or explicitly disable it for tests). Module registration can **fail** at startup if configuration is invalid; check the error from `module.RegisterWiring(app)` and `module.RegisterTransports(app)` or use `application.ApplyProfile(...)`.
