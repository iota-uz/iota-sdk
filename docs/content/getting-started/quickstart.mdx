...
3. **Apply a Profile** - Use `application.ApplyProfile(ctx, app, application.CompositionProfileServer, yourModule)` or call `application.Wire(...)`, `application.RegisterTransports(...)`, and `app.StartRuntime(...)` directly when you need finer control
