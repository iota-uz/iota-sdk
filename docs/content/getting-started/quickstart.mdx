...
2. **Register Components** - In your module's `Register(app)`, register services, controllers, nav items
3. **Load Module** - Pass your module to `modules.Load(app, yourModule)` (built-in modules are already loaded via `modules.Load(app, modules.BuiltInModules...)`; do not edit `BuiltInModules` to add custom modules)
