...
2. **Split Phases** - Put services, locales, assets, and runtime registrations in `RegisterWiring(app)`; put controllers and GraphQL/WebSocket setup in `RegisterTransports(app)`
3. **Compose Explicitly** - Call `application.Wire(app, yourModule)`, then `application.RegisterTransports(...)` when the process serves APIs, and `app.StartRuntime(...)` with explicit runtime tags when it should run long-lived background components
