...
db cmd="help" direction="":
  case "{{cmd}}" in \
    ...
    *) \
      echo "Usage: just db [local|stop|clean|reset|seed|migrate] [up|down|redo|status]" ; \
      exit 2 ;; \
  esac
...
e2e cmd="help" direction="":
  case "{{cmd}}" in \
    ...
    *) \
      echo "Usage: just e2e [test|reset|seed|migrate|run|ci|dev|clean] [up|down|redo|status]" ; \
      exit 2 ;; \
  esac
