...
func withSessionStatus(status domain.SessionStatus) sessionOption {
	return func(f *sessionFixture) {
		f.status = status
		f.statusSet = true
	}
...
func withSessionParentID(parentID uuid.UUID) sessionOption {
	return func(f *sessionFixture) {
		id := parentID
		f.parentSessionID = &id
	}
}
...
func withSessionCreatedAt(createdAt time.Time) sessionOption {
	return func(f *sessionFixture) { f.createdAt = createdAt }
}

func withSessionUpdatedAt(updatedAt time.Time) sessionOption {
	return func(f *sessionFixture) { f.updatedAt = updatedAt }
}
