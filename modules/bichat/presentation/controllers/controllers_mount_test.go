...
type controllerDeps struct {
	...
	conversationSvc   pkgservices.ConversationService
	streamService     pkgservices.StreamService
	hitlService       pkgservices.HITLService
...
func mustCreateSession(t *testing.T, ctx context.Context, deps controllerDeps, tenantID uuid.UUID, u coreuser.User, title string) bichatdomain.Session {
	...
	s, err := deps.sessionService.CreateSession(context.WithValue(ctx, constants.TxKey, nil), tenantID, int64(u.ID()), title)
...
func TestControllers_BasePathRouting_Integration(t *testing.T) {
	...
	NewChatController(env.App, deps.sessionService, deps.conversationSvc, deps.hitlService, deps.chatRepo, deps.agentService, nil, nil,
...
func TestStreamController_RequireAccessPermission_Integration(t *testing.T) {
	...
	r := newRouterWithContext(t, env, u)
	NewStreamController(env.App, deps.streamService, deps.sessionService, deps.attachmentService, WithRequireAccessPermission(bichatperm.BiChatAccess)).Register(r)
...
func TestStreamController_DebugMode_AllowedWithoutExtraPermission_Integration(t *testing.T) {
	...
	r := newRouterWithContext(t, env, u)
	NewStreamController(env.App, deps.streamService,
		deps.sessionService, deps.attachmentService,
		WithRequireAccessPermission(bichatperm.BiChatAccess),
	).Register(r)
