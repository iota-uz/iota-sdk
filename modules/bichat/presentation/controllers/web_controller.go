...
// NoOpMetricsRecorder is a no-op implementation of MetricsRecorder for BiChat
...
// RecordDuration implements MetricsRecorder (no-op)
func (n *NoOpMetricsRecorder) RecordDuration(name string, duration time.Duration, labels map[string]string) {
	// No-op
}

// IncrementCounter implements MetricsRecorder (no-op)
func (n *NoOpMetricsRecorder) IncrementCounter(name string, labels map[string]string) {
	// No-op
}
...
func (c *WebController) RenderChatApp(w http.ResponseWriter, r *http.Request) {
	...
	config := applet.Config{
		...
		Endpoints: applet.EndpointConfig{
			GraphQL: "/bi-chat/graphql",
			Stream:  "/bi-chat/stream",
		},
	}

	// Create session config for applet context
	...
	// Create metrics recorder (use NoOpMetricsRecorder since BiChat doesn't have metrics service)
	metrics := NewNoOpMetricsRecorder()

	// Build initial context for React using the applet package with new production API
	// This extracts user, tenant, locale, and config from request context
	builder := applet.NewContextBuilder(
		config,
		c.app.Bundle(),
		sessionConfig,
		logger,
...
<head>
    ...
    <title>BI Chat</title>
    <script type="module" src="/bi-chat/assets/index.js"></script>
</head>
<body>
    <div id="app"></div>
    <script>
