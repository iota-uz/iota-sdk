...
func (g *GormDialogueRepository) Count(ctx context.Context) (int64, error) {
	...
	if err := tx.Model(&dialogue.Dialogue{}).Count(&count).Error; err != nil { //nolint:exhaustruct
		====== =
		func(g *GormDialogueRepository) GetPaginated(ctx
		context.Context, params * dialogue.FindParams) ([]dialogue.Dialogue, error) {
			var args []interface{}
			where := []string{"1 = 1"}
		...
		func(g *GormDialogueRepository) Count(ctx
		...
		<<<<<<< Updated
		upstream
		func(g *GormDialogueRepository) GetAll(ctx
		context.Context) ([]*dialogue.Dialogue, error) {
			tx, ok := composables.UseTx(ctx)
			if !ok {
				return nil, composables.ErrNoTx
			}
		...
		func(g *GormDialogueRepository) GetByID(ctx
		context.Context, id
		int64) (*dialogue.Dialogue, error) {
			tx, ok := composables.UseTx(ctx)
			if !ok {
				return nil, composables.ErrNoTx
			}
		...
		func(g *GormDialogueRepository) GetByUserID(ctx
		...
		func(g *GormDialogueRepository) Create(ctx
		context.Context, data * dialogue.Dialogue) error{
			tx, ok, := composables.UseTx(ctx)
			if !ok{
		...
		func(g *GormDialogueRepository) Update(ctx
		context.Context, data * dialogue.Dialogue) error{
			tx, ok, := composables.UseTx(ctx)
			if !ok{
			return composables.ErrNoTx
		...
		func(g *GormDialogueRepository) Delete(ctx
		context.Context, id
		int64) error{
			tx, ok := composables.UseTx(ctx)
			if !ok {
