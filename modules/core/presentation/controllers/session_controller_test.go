...
func newAccountController(suite *itf.Suite) application.Controller {
	...
	httpCfg := itf.GetService[httpconfig.Config](env)
	return controllers.NewAccountController(env.App, userService, tenantService, uploadService, sessionService, httpCfg)
}
...
func TestAccountController_GetSessions(t *testing.T) {
	...
	t.Run("Returns_Sessions_List_For_Authenticated_User", func(t *testing.T) {
		...
		cfg := itf.GetService[httpconfig.Config](suite.Env())
		...
		response := suite.GET("/account/sessions").
			Cookie(cfg.Cookies.SID, currentToken).
			Expect(t)
...
func TestSessionController_RevokeUserSession(t *testing.T) {
	...
	t.Run("Returns_403_Without_SessionDelete_Permission", func(t *testing.T) {
		...
		controller := controllers.NewSessionController("/sessions", itf.GetService[httpconfig.Config](suite.Env()))
		suite.Register(controller)
